...
class CapsuleService : Service() {
    ...
    override fun onCreate() {
        ...
        provider = when {
            ...
            MiuiIslandManager.isHyperOsWithOfficialFocusSupport() -> {
                Log.d(TAG, "Using HyperOS official focus notification")
                FlymeCapsuleProvider() // 使用相同接口
            }
            else -> NativeCapsuleProvider()
        }
        startObservingCapsuleState()
        Log.d(TAG, "CapsuleService created - Provider: ${provider.javaClass.simpleName}")
    }
